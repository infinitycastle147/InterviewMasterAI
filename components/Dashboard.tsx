...
import { RefreshCw, FileText, Play, Code2, AlertCircle, Settings2, Hash, ChevronRight, Github } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {files.length === 0 ? (
                  <div className="text-center py-12 px-4 bg-gray-800/30 rounded-2xl border border-gray-800 border-dashed">
                      ...
                      <button 
                         onClick={() => { setUseDemo(true); setSelectedFile(null); }}
                  ...
                  files.map((file, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${selectedFile === file && !useDemo ? 'bg-indigo-500/20 text-indigo-400' : 'bg-gray-800 text-gray-600'}`}>
                           <Hash className="w-4 h-4" />
                        </div>
                        <span className="font-semibold">{file.name}</span>
                      </div>
                      {selectedFile === file && !useDemo && <ChevronRight className="w-4 h-4 text-indigo-400" />}
                    </button>
                  ))
                )}
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <div className="bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-3xl p-8 shadow-2xl flex-1 flex flex-col">
              ...
              <button
                onClick={handleStart}
                disabled={(!selectedFile && !useDemo) || maxQuestions === 0}
                ...
                (!selectedFile && !useDemo) || maxQuestions === 0
